...
- Primary remote: `git@github.com:xixi-nearing/miniRPC.git`
- Default branch: `main`

Future update rule for agents working in `/home/developer/miniRPC`:
- After completing a requested change and running the appropriate verification, stage the relevant files, create a descriptive commit, and push the latest `main` branch changes to `origin`.
- If the push is blocked by authentication, network issues, or remote conflicts, report the blocker clearly and stop short of destructive history rewriting unless the user explicitly asks for it.
- Do not leave completed local commits unpushed unless the user explicitly asks not to push.
</repo_sync_policy>
